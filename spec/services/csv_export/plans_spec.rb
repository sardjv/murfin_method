describe CsvExport::Plans do
  ...
  let(:plan1) { create :plan, user: user1, start_date: 1.year.ago, end_date: Date.tomorrow }
  ...
  let(:plan2) { create :plan, user: user2, start_date: 6.months.ago, end_date: 6.months.since }
  ...
  it 'returns csv' do
    ...
    expect(data[1]).to eql [user1.first_name, user1.last_name, plan1.start_date.to_s, plan1.end_date.to_s, 'Draft', "14h 30m","DCC", "Care Co-ordination", "Yes", "8h"]
    expect(data[2]).to eql [user1.first_name, user1.last_name, plan1.start_date.to_s, plan1.end_date.to_s, 'Draft', "14h 30m","SPA", "Initial Assessment", "No", "6h 30m"]
    expect(data[2]).to eql [user2.first_name, user2.last_name, plan2.start_date.to_s, plan2.end_date.to_s, 'Draft', "7h","DCC", "Team Meetings", nil, "7h"]
  end

  # TODO plan with no activities
end