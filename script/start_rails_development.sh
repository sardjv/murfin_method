...
then echo "$FIRST_RUN_FLAG_FILE detected, skipping library installation..."
  ...
  RAILS_ENV=test MYSQL_USER=root MYSQL_PASSWORD=$MYSQL_ROOT_PASSWORD bundle exec rails parallel:setup # Create 1 test database per core.
  RAILS_ENV=development bundle exec rails db:create db:migrate

  echo "This is an autogenerated file that is created when Murfin+ is run for the first time. It's used as a flag to speed up boot times after the initial boot." > $FIRST_RUN_FLAG_FILE
fi
