...
class GenerateUsersCsvJob < ApplicationJob
  ...
  def perform(*args) # rubocop:disable Metrics/MethodLength, Metrics/AbcSize
    ...
    begin
      ...
      link_html = link_to I18n.t('actions.download'), path, data: { turbolinks: false }

      FlashMessageBroadcastJob.perform_now(
        current_user_id: current_user_id,
        message: raw(I18n.t('download.ready', file_type: 'CSV', link: link_html)), # rubocop:disable Rails/OutputSafety
        alert_type: 'success',
      ...
      FlashMessageBroadcastJob.perform_now(
        current_user_id: current_user_id,
        message: I18n.t('download.processing_error', file_type: 'CSV'),
        alert_type: 'danger',
        extra_data: { message_type: 'download' }
      )
    ensure
